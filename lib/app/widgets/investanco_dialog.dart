...
/// Visual weight of an [InvestancoDialogAction] button. Drives which button
/// style (and colour) the dialog renders.
enum InvestancoDialogActionKind {
...
/// One button in an [InvestancoDialog] footer.
///
...
class InvestancoDialogAction {
  /// Creates a footer action.
  const InvestancoDialogAction({
    required this.label,
    required this.onPressed,
    this.kind = InvestancoDialogActionKind.secondary,
  });
  ...
  /// Visual weight (style + colour).
  final InvestancoDialogActionKind kind;
}
...
/// Mirrors financo's `FinancoDialog`.
///
/// Layout of [actions]:
...
/// );
/// ```
class InvestancoDialog extends StatelessWidget {
  /// Creates a dialog. [actions] must not be empty.
  const InvestancoDialog({
    required this.title,
  ...
  final List<InvestancoDialogAction> actions;
  ...
  /// Caps the dialog width on wide layouts.
...
/// Tinted circular glyph badge shown above the dialog title.
...
/// Footer button group; layout adapts to the number of [actions].
class _ActionButtons extends StatelessWidget {
  ...
  Widget _button(BuildContext context, InvestancoDialogAction action) {
    final colors = context.appColors;
    final label = Text(action.label);
    switch (action.kind) {
      case InvestancoDialogActionKind.primary:
        return FilledButton(onPressed: action.onPressed, child: label);
      case InvestancoDialogActionKind.destructive:
        return FilledButton(
      ...
      case InvestancoDialogActionKind.secondary:
...
Future<bool> showInvestancoConfirmDialog(
    ...
    builder: (ctx) => InvestancoDialog(
      ...
      actions: [
        ...
        InvestancoDialogAction(
          label: confirmLabel,
          kind: destructive
              ? InvestancoDialogActionKind.destructive
              : InvestancoDialogActionKind.primary,
          onPressed: () => Navigator.pop(ctx, true),
        ),
      ],
    ),
...
Future<void> showInvestancoMessageDialog(
  BuildContext context, {
  required String title,
  required String message,
  String? buttonLabel,
